...
_LANGUAGES = [
    ("auto", "Otomatik (TR + EN karisik icerik icin)"),
    ("tr", "Turkce (zorla)"),
    ("en", "Ingilizce (zorla)"),
]

_TIMESTAMP_MODES = [
    ("interval", "Belirli araliklarla"),
...
_SUB_POLICIES = [
    ("ask", "Sor (onerilen)"),
    ("always", "Her zaman hazir altyaziyi kullan"),
...
class SettingsDialog(QDialog):
    ...
    def _transcription_tab(self) -> QWidget:
        ...
        self.model_combo.addItem("Otomatik (donanima gore sec)", "auto")
        ...
        for name, spec in MODEL_CATALOG.items():
            ...
            if reason:
                ...
                self.model_combo.setItemData(
                    index, f"{spec.label} (kullanilamiyor: {reason})", Qt.ItemDataRole.ToolTipRole
                )
        ...
        hint = QLabel(
            "Video tek dilliyse dili elle zorlamak hem daha hizli hem daha dogru sonuc verir."
        )
        ...
        form.addRow("Is parcacigi", self.threads_spin)

        self.low_memory_check = QCheckBox(
            "Dusuk bellek modu (yigin 1, kucuk pencere, kucuk model)"
        )
        self.low_memory_check.setChecked(self._settings.low_memory_mode)
        self.low_memory_check.setToolTip(
            "Ayni anda baska agir isler calistiracaksaniz veya bellek yetersizligi "
            "yasadiysaniz acin. Islem yavaslar ama guvenli tarafta kalir."
        )
    ...
    def _models_box(self) -> QWidget:
        box = QGroupBox("Indirilmis modeller")
        ...
        delete = QPushButton("Secili modeli sil")
        delete.clicked.connect(self._delete_model)
        delete.setToolTip("Yer acmak icin. Tekrar gerektiginde otomatik iner.")
    ...
    def _system_tab(self) -> QWidget:
        ...
        form.addRow("Hazir altyazi varsa", self.subtitle_combo)
        note = QLabel(
            "Insan eliyle yazilmis altyazi 3 saatlik islemi 2 saniyeye indirir. "
            "Otomatik uretilmis altyazilar Turkcede noktalama tasimadigi icin kullanilmaz."
        )
        ...
        form.addRow("Tarayici cerezi", self.cookie_combo)
        cookie_note = QLabel(
            "Yas kisitli veya ozel videolar icin gerekir. Tarayicinin kapali olmasi gerekebilir."
        )
        ...
        warning = QLabel(
            "YouTube sik degistigi icin yt-dlp zamanla bozulur. Link indirmede hata "
            "aliyorsaniz once burayi guncelleyin."
        )
    ...
    def _refresh_models_label(self) -> None:
        ...
        if not downloaded:
            self.models_label.setText(
                "Henuz model inmedi. Ilk is baslatildiginda otomatik olarak inecek."
            )
            return
        parts = []
    ...
    def _delete_model(self) -> None:
        downloaded = catalog.downloaded_models()
        if not downloaded:
            QMessageBox.information(self, "Model yok", "Silinecek indirilmis model yok.")
            return
        ...
        answer = QMessageBox.question(
            self,
            "Modeli sil",
            f"{target} modeli diskten silinsin mi? Tekrar gerektiginde yeniden iner.",
        )
    ...
    def _refresh_ytdlp_label(self) -> None:
        ...
        source = "kullanici dizini" if ytdlp_update.is_user_managed() else "uygulamayla geldi"
        self.ytdlp_label.setText(f"Kurulu surum: {version} ({source})")

    def _update_ytdlp(self) -> None:
        dialog = QProgressDialog("yt-dlp guncelleniyor...", "Vazgec", 0, 100, self)
        dialog.setWindowTitle("Guncelleme")
        ...
        try:
            ...
            dialog.close()
            QMessageBox.warning(self, "Guncellenemedi", str(exc))
            return
        dialog.close()
        QMessageBox.information(
            self,
            "Guncellendi",
            f"yt-dlp {version} kuruldu. Degisiklik uygulama yeniden baslatildiginda etkinlesir.",
        )
        self._refresh_ytdlp_label()

    def _pick_output_dir(self) -> None:
        directory = QFileDialog.getExistingDirectory(
            self, "Cikti klasoru", self.output_edit.text()
        )
        if directory:
